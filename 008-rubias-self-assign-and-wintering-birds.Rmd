...
write_rds(tdf, "stored_run_times/007.rds")
tdf
``` 
...
```{r}
# down here, temporarily, I am just putting the assignments into the winter-assignments
# appendix
winter_assig$indiv_posteriors %>%
  ...
  left_join(winter_meta, ., by = c("Field_Number" = "indiv")) %>%
  select(
    Field_Number:Collector.y,
    PNW,
    KER,
    SCC,
    INW,
  ...
  write_csv("data/appendices/app1-winter-indivs-assignments-and-genos.csv")
```
