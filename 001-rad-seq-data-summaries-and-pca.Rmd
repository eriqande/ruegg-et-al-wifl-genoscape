...
```{r}
meta_full <- read_csv("data/wifl-samples-metadata.csv")

inw_ids <- meta_full %>%
  filter(Collection.Location == "INW", Screen == "RAD") %>%
  pull(indiv)
...
```{r}
meta <- meta_full %>%
  select(indiv, Collection.Location)

pca2 <- pca$pca_df %>%
  left_join(meta, by = c("sample" = "indiv"))
...
```{r}
FST <- meta %>%
  filter(indiv %in% rownames(rad3)) %>%
  rename(sample = indiv, group = Collection.Location) %>%
  pairwise_fst(rad3, .)
```
