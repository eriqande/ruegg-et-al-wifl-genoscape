...
We also attach some other names to them, for the structure runs, etc.
...
```
The dashed line there shows a 10% cutoff, which is what we applied to toss out other loci.

Here are the loci that we toss:
```{r}
toss_em <- miss_rates %>%
  filter(miss_fract > 0.1) %>%
  arrange(desc(miss_fract))
toss_em
```
Remove those to make our final data set of Fluidigm-typed breeders and write it out.
```{r}
final_fluidigm_breeders_182_loci_393_birds_long <- fluidigm_breeders_long %>%
  anti_join(toss_em, by = "CHROM")
write_rds(
  final_fluidigm_breeders_182_loci_393_birds_long,
  file = "outputs/003/final_fluidigm_breeders_182_loci_393_birds_long.rds",
  compress = "xz"
)
```
